...
VJUsage_Step3 <- function(patient, cohort, timepoint_order, fig_height, fig_width, fig_path,
    ...
    ########################################################################
    #Color palette:
    col_num <- 500 #default number of colors. Change it if you have less 
                  #or more cassettes that encode more than 2 CDR3s

    color_pal <- randomColor(count =  col_num )
    ######################################################################## 
    ...
    CDR3_colors <- read_csv(
            file.path(
                    CDR3_colors_datapath,
                    CDR3_colors_filename)
            ) %>%
