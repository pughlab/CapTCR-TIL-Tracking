...
# @param dir_data: Data where all data files are ('mixcr_output.xlsx', 'SampleKeys.xlsx', )
...
Initialize <- function(dir_data, timepoint_order){
  ...
  # Creating set of timepoint lists using Sample Keys data
  samplekeys <- read_excel(paste(dir_data, "SampleKeys.xlsx", sep=""))
  
  # Creating list of patient IDs
  patient_list <- unique(paste(samplekeys$Project, samplekeys$Patient_ID, samplekeys$Sample_Cohort, sep="_"))
  ...
  for(patient in patient_list){
    ...
    Total <- data.frame()
    for(timepoint in 1:length(timepoint_list_unorder)){
      filenme <- unique(patient_mixcr$filename)[which(grepl(patient_samplekeys$Informatics_Name[timepoint], unique(patient_mixcr$filename))==TRUE)]
      timepoint_mixcr <- MiXCR_output[which(MiXCR_output$filename==filenme),]
      timepoint_mixcr$filename <- timepoint_list_unorder[timepoint]
      
      # Subseting dataframe to necessary clonal and CDR3 data
      preCDR3_fraction <- as.data.frame(timepoint_mixcr[, c("filename","aaSeqCDR3","cloneFraction", "cloneCount")]) 
      ...
      totalClone <- sum(preCDR3_fraction$cloneCount)
      preCDR3_fraction$cloneFraction <- preCDR3_fraction$cloneCount/totalClone
      
      assign(paste(patient, timepoint_list_unorder[timepoint], sep="_"), preCDR3_fraction, envir = .GlobalEnv)
    
      # Creating total dataframe
      Total <- rbind(Total, eval(as.name(paste0(patient, "_", timepoint_list_unorder[timepoint]))))
    }
