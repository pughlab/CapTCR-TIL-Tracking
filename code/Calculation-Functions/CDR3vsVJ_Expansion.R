...
CDR3vsVJ_expansion <- function(patient, cohort, clones, comp_samp){

        # Creates dataframe comparing a CDR3's growth and its corresponding VJ-growth
        timepoint_order <- c("baseline", "infusion", "4W")
        for(patient in patients){
            ...
            clonotype <- clonotype[-c(grep("[*]", clonotype$aaSeqCDR3)),]
            clonotype <- clonotype[-c(grep("_", clonotype$aaSeqCDR3)),]
            for(clone in 1:length(clonotype$cloneFraction)){
              clonotype$cloneFraction[clone] <- clonotype$cloneCount[clone]/sum(clonotype[which(clonotype$Cycle==clonotype$Cycle[clone]),]$cloneCount)
            }

            inf_df <- clonotype[which(clonotype$Cycle=="infusion"),]
            bl_df <- clonotype[which(clonotype$Cycle=="baseline"),]
            if(clones == "TIL"){
            ...
            if(patient=="TLML_1_"){
                fw_df <- clonotype[which(clonotype$Cycle=="FU_01"),]
            } else{
                fw_df <- clonotype[which(clonotype$Cycle=="4W"),] 
            }
